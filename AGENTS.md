...
- 默认只修改用户明确点名的机制、数值、bug、文件或文案 key。不要因为“顺手统一”“顺手补完”“看起来更好”扩大范围。
- 文案修改只处理当前需求必需的缺失、错误或指定表达。不要擅自润色已有中文/日文文本，不要把一种语言的表达风格同步改到另一种语言。
- UI 结构、折叠项、布局、动画和展示密度属于高风险改动；除非用户明确要求 UI 调整，或修复当前 bug 必须触碰，否则不要改。
- 如果发现额外问题或认为某个额外改动有价值，先向用户说明原因和影响，等确认后再实施。
- 排查缓存、强刷、本地化加载等问题时，优先验证加载链路和缓存状态，不要用它作为理由重写文案或 UI 呈现。
