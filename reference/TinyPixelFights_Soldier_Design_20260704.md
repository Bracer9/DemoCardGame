...
每个己方 turn 1 次，主动攻击没有共有盾保护的敌人前，50% 赋予与目标攻击类型对应的 2 turn 力竭或磨损。该效果在攻击宣言时结算，不要求本次攻击造成 HP 伤害；攻击有共有盾保护的敌人不会触发，也不会消耗本 turn 的触发机会。

### Rank1

- Max HP +2。
- 若主目标在触发前已经拥有任意 Debuff，同时对每个相邻敌人独立进行一次 50% 判定，赋予与该目标攻击类型对应的力竭或磨损。
