...
更新时间：2026-07-11
状态：25 件目标池已实装；目录、效果、宽松英雄 tag 权重、双语文本、日志与攻击预测已同步。
...
- 9 件 Epic 终局遗物，对应 9 条不同的构筑终点。
- 每个体系只占 1 件专属终局遗物；成型过程尽量借用共通组件。
...
| 体系 | 主要英雄路线 | 士兵 / 副官 | 共用地基与组件 | 唯一终局遗物 |
|---|---|---|---|---|
...
| Debuff 收割 | Fate Dealer、Arcane Archivist、Wildspeaker、Dragon Raider | Arcanist、Duelist | 女巫铃、空心彗镜、夜饵；燃烧也计入 Debuff | 瘟疫法典 |
...
| 绿旗<br>`relic-green-standard` | 5 BP | `physical` `shield` `ap` | 每回合首次击破敌方共有盾后，返还 1 AP。 | 共有盾仍吸收本次全部伤害；返还 AP 只用于后续行动，且不超过 AP 上限。 | 已按 AP 效果接入 |
...
| 结算 | 在同一个攻击伤害包中增加数值，仍先扣士气；不是追加攻击或第二段伤害，共有盾破碎时仍不穿透。 |
...
| Debuff 收割 | 女巫铃 + 空心彗镜 + 瘟疫法典 | 夜饵、余烬星盘、各英雄自带战栗 / 脆弱 / 力竭 / 磨损 |
...
- `RelicCatalog.All` 恰好为 25 件：6 件原样保留、4 件保留 ID 并调整效果、15 件新增、7 件旧遗物下架。
...
- 自动检查覆盖 25 个唯一 ID、双语 25 项一一对应、关键事件 hook 与旧遗物运行代码清理。
