...
当前共 5 名 `Soldier / 士兵`，已实装 Rank0 到 Rank2 的基础成长、进阶立绘和 Rank2 Role Action。
...
| `jester` | `malicious-jest` | Rank0/1每回合1次：无盾攻击前50%赋予对应力竭/磨损，造成HP伤害则必定赋予；Rank2改为确定赋予；HP+2，已有Debuff时向相邻敌人扩散，Debuff目标主动攻击伤害+1 | `mocking-curtain-call` |
...
- `Domain/CharacterDefinition.cs`：维护 5 名士兵定义、`Hero/Soldier` 分类入口、Rank2 立绘字段。
- `Domain/GameState.cs`：新增士兵 Rank、士兵选择 draft 状态。
- `Domain/GameEngine.cs`：开局士兵选择、奖励招募、强化升阶、Rank 属性与 Rank2 Role Action 解锁。
- `Domain/Traits.cs`：维护 5 个士兵 Trait。
- `Domain/RoleActions.cs`：注册 5 个士兵 Rank2 Role Action。
...
- `Domain/DeputyDefinitions.cs` / `Domain/GameEngine.cs`：接入 `deputy-jester`，包括宿主敌方行动后的输出削弱。
- `assets/New_Portraits/Jester.png` / `MasqueJester.png`：弄臣 Rank0 与 Rank2 立绘。
