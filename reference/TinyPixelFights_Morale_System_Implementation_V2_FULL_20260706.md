...
决斗嗅觉在决斗者每个己方 turn 第一次主动攻击对敌人造成 HP 伤害后提供强攻。

士气后：

```text
因为触发条件看真实 HP 伤害，只打士气不触发决斗嗅觉。
触发后获得的强攻影响后续主动物理伤害：先放大伤害，再进入士气。
```

Rank1 / 决斗副手额外绝对伤害：

```text
主动攻击后若目标仍存活，追加 2 点绝对伤害。
属于绝对伤害，直接扣 HP，不经过士气，也不要求本次攻击造成 HP 伤害。
```
