...
副官不是新的英雄成长树，也不是新的 Role Action。V1 不做“每名英雄 × 每类士兵”的专属相性表。
...
- 不再能成为敌人目标；
- 不再承伤；
- 不再触发原本作为独立士兵时的主动 / 事件型 Trait；
- Rank1 持续光环继续生效，光环来源改为绑定宿主；宿主存活并在战场时有效；
- 保留副官加成与副官特性。
```
...
不要给各副官分别新增 `cleric-deputy-buff`、`shieldmaiden-deputy-buff` 等状态。
...
| 弄臣 / Masque Jester | `deputy-jester` | 攻击 +1 | 每己方 turn 1 次，宿主主动攻击敌人，或以 Role Action 指定敌人并成功结算后，若主目标仍存活，根据主目标的攻击类型赋予 2 turn 力竭或磨损。 | 力竭 / 磨损 |

弄臣与 `deputy-jester` 当前为新增设计案，尚未接入源码；其余四类副官保持现有实装。
...
- 弄臣副官在宿主原行动完整结算后触发，只处理本次主动攻击或 Role Action 的主目标；主目标已经被击败时不触发。物理攻击单位获得力竭，魔法攻击单位获得磨损。
- 弄臣副官不要求目标原本已有 Debuff，也不要求本次行动造成 HP 伤害；它是通用 Debuff 启动件，不是 Fate Dealer 专属效果。
- 副官本体和基础数值加成不可驱散。
...
不做逐英雄、逐士兵的专属表，是为了避免系统爆炸。

每类士兵只提供一个所有英雄通用的副官模板：
...
| 狂战士 | 盾卫、牧师、决斗士、弄臣 | 盾卫 / 牧师补偿战吼后的防御风险；决斗士提高收割能力；弄臣让挑战与主动攻击继续补充输出削弱。 |
| 魔法师 | 秘术士、盾卫、弄臣 | 秘术士通过魔法伤害、燃烧、空虚给魔法师魔涌；盾卫保护蓄能窗口；弄臣让灼热刻印与主动攻击补充力竭 / 磨损。 |
| 占卜师 | 秘术士、决斗士、弄臣 | 秘术士放大魔法 / 状态节奏；决斗士负责后续 HP 收割；弄臣让命运标记与切线稳定接入通用 Debuff。 |
| 怪物 | 决斗士、牧师 | 决斗士放大怪物对 Debuff 目标的爆发；牧师提升魔防，帮助承受魔法压力。 |
| 农民 | 盾卫、决斗士、牧师 | 农民补给篮和坚守体系适合盾卫；决斗士配合低费收割；牧师强化治疗支援。 |
| 德鲁伊 | 牧师、决斗士、秘术士、弄臣 | 德鲁伊净化触发牧师；力竭 / 磨损给决斗士创造目标；施加状态可触发秘术士；弄臣帮助控制与士兵路线扩大 Debuff 覆盖。 |

弄臣副官不锁定 Fate Dealer。任何会持续主动攻击敌人，或拥有敌方目标 Role Action 的英雄都能稳定触发；Fate Dealer 只是因为命运标记与 `thread-cut` 会消费 Debuff 密度而具有更高协同。
...
| `deputy-jester` | `deputy`, `debuff`, `control`, `attack`, `role-action`, `soldier` |
...
| 弄臣 | `OnActiveAttackResolved`、`OnRoleActionResolved`，检查行动者是否为宿主、主目标是否为存活敌人；根据目标当前攻击类型赋予力竭或磨损。 |
...
| 弄臣 | 任意英雄带弄臣副官，主动攻击仍存活的物理敌人 | 原攻击结算后，该敌人获得 2 turn 力竭。 |
| 弄臣 | Fate Dealer 带弄臣副官，对魔法敌人使用命运标记 | 命运标记结算后，该敌人获得 2 turn 磨损；该效果不是 Fate Dealer 专属规则。 |
...
| 副官触发产生强攻 / 魔涌 / 坚守 / 护咒 / 力竭 / 磨损 | 这些状态正常进入对应 Buff / Debuff 显示组。 |
...
1. 英雄与副官的推荐相性加成，但不要为每种组合编写全专属规则。
2. 副官可以跨战斗保存。
