...
副官：秘术士
永久加成：魔防 +1
副官特性：每己方 turn 1 次，宿主造成魔法伤害或赋予燃烧 / 空虚 / 力竭 / 磨损后，获得 1 层咏唱。
Role Action：灼热刻印
```
...
| 副官触发产生的状态 | 使用现有通用状态，例如强攻、咏唱、坚守、护咒。 |
...
|---|---|---|---|---|
| 牧师 / Saint Cleric | `deputy-cleric` | 最大与当前 HP +6 | 每己方 turn 1 次，宿主的 Role Action 造成有效治疗，或宿主的 Role Action 成功净化 1 个 Debuff 后，被治疗 / 被净化目标获得护咒。若目标当前 HP 低于一半，护咒持续改为 2 turn。 | 护咒 |
...
| 决斗士 / Crimson Duelist | `deputy-duelist` | 攻击 +2 | 每己方 turn 1 次，宿主主动攻击带有通用 Debuff 的敌人时，伤害计算前获得 1 turn 强攻。若目标已有 2 个及以上通用 Debuff，强攻持续改为 2 turn。 | 强攻 |
| 秘术士 / Astral Arcanist | `deputy-arcanist` | 魔防 +2 | 每己方 turn 1 次，宿主造成魔法伤害，或宿主成功赋予燃烧 / 空虚 / 力竭 / 磨损后，宿主获得 1 层咏唱。若由宿主的 Role Action 触发，改为获得 2 层咏唱。 | 咏唱 |

说明：

- “宿主”指被选为副官目标的英雄。
- “通用 Debuff”指燃烧、脆弱、空虚、力竭、磨损、战栗等通用 Debuff。V1 不包括命运标记、猎物、黑暗契约等特殊状态。
- 副官特性触发产生的强攻、咏唱、坚守、护咒仍是普通 Buff，默认可驱散。
- 副官本体和基础数值加成不可驱散。
...
| 魔法师 | 秘术士、盾卫 | 秘术士通过魔法伤害、燃烧、空虚给魔法师咏唱；盾卫保护蓄能窗口。 |
| 占卜师 | 秘术士、决斗士 | 秘术士放大魔法 / 状态节奏；决斗士配合命运标记后的集火窗口，但命运标记本身不算通用 Debuff。 |
| 怪物 | 决斗士、牧师 | 决斗士放大怪物对 Debuff 目标的爆发；牧师增加最大 HP，帮助承受献祭和反噬风险。 |
...
"deputy": {
  ...
  "statText": "魔防 +1",
  "passiveText": "每己方 turn 1 次，宿主造成魔法伤害或赋予燃烧 / 空虚 / 力竭 / 磨损后，获得 1 层咏唱。"
}
```
...
| 秘术士 | 魔法师带秘术士副官，使用灼热刻印赋予燃烧 / 空虚 | 魔法师获得咏唱。 |
...
| 副官触发产生强攻 / 咏唱 / 坚守 / 护咒 | 这些状态正常进入 Buff 显示组。 |
