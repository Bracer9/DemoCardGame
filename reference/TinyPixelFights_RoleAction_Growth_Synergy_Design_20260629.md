...
| `SharedShield` | 玩家共享盾。存在共享盾时，本次普通伤害先打盾，盾破不穿透到 HP。 |
...
- 通过非开局奖励获得的新英雄 / 士兵进入“部署中”：不能主动攻击，不能使用 Role Action，不能被敌我双方攻击、Role Action、Trait、治疗、Buff 指定。由于奖励 round 会被跳过，部署中按 2 次拥有者己方 turn start 结算。
- 开局士兵选择当前只选择 1 名士兵。
...
| Duelist | 我方物理攻击单位攻击 +2。 | `crimson-lunge` 施加战栗，若目标已有通用 Debuff 再施加脆弱；副官让宿主攻击无盾目标时获得决斗嗅觉。 |
| Arcanist | 我方魔法攻击单位攻击 +2。 | `astral-focus` 给友方咏唱或给敌方空虚；副官让宿主获得咏唱并可再行动一次。 |
...
| Rank3 | MaxHp +4，PhysicalDefense +1，HP 全回复。解锁 `miracle-standard`：2 AP，选择我方 1 体，主目标治疗 3 HP 并净化 1 个 Debuff；相邻我方各治疗 1 HP。若本行动至少治疗或净化 1 次，我方共享盾 +1。Cooldown 1。 |

平衡边界：

- 不回复士气；治疗只修 HP。
- Rank2 强化只看真实 HP 回复或成功净化，不能通过满血空治疗刷触发。
- `miracle-standard` 的共享盾 +1 是结果奖励，不因相邻目标数量叠加。
...
| Rank3 | MaxHp +3，Attack +1，HP 全回复。解锁 `edict-of-victory`：1 AP，选择我方 1 体，使其本 turn 可再次主动攻击；该目标本 turn 第一次造成敌方 HP 伤害时，我方获得 1 BP，受每 turn BP 上限限制；下个己方 turn AP debt +1。Cooldown 1。 |

平衡边界：

- `edict-of-victory` 不直接加伤害，强度来自 AP 与目标选择。
- BP 奖励只看真实 HPDamage，只打士气不奖励。
...
| Rank3 | MaxHp +2，Attack +1，HP 全回复。解锁 `astral-alignment`：1 AP，选择我方魔法单位，获得 1 层咏唱；若目标本 turn 已经主动攻击过，再获得 1 次主动攻击机会。该目标本 turn 第一次造成敌方 HP 伤害时，我方获得 1 BP。Cooldown 1。 |

平衡边界：

- 咏唱不回复士气，不改变 HPDamage 判定。
- BP 奖励只看真实 HP 伤害，受上限 5 限制。
- 对 Arcanist 副官的“咏唱 + 再行动”有强协同，Rank3 实装时需要同 turn 额外攻击次数展示清楚。
...
| Rank3 | MaxHp +3，MagicalDefense +1，HP 全回复。解锁 `thread-cut`：1 AP，选择敌方 1 体。若目标有 `marked`、`prey`、`void`、`exhaustion`、`erosion` 中任意状态，造成 2 点魔法伤害并施加 1 turn 战栗；否则只施加 `marked`。Cooldown 1。 |

平衡边界：

- `thread-cut` 的 2 点魔法伤害是普通伤害，先扣士气。
...
| Rank3 | MaxHp +4，MagicalDefense +1，HP 全回复。解锁 `field-rations`：1 AP，治疗所有我方非部署中角色 1 HP；选择 1 个主目标额外获得 1 turn 坚守和 1 turn 护咒。Cooldown 1。 |

平衡边界：

- `supply-basket` 和 `field-rations` 都只治疗 HP。
- Rank3 全队治疗量小，强度来自同时触发 Cleric / Shieldmaiden 体系。
...
| Rank3 | MaxHp +3，Attack +1，HP 全回复。解锁 `militia-call`：1 AP，选择我方 Peasant 或 Soldier，目标获得 1 turn 坚守并可再次主动攻击；若目标本 turn 击败敌方角色，我方获得 1 BP。Cooldown 1。 |

平衡边界：

- Rank2 / Rank3 BP 都只看真实 HPDamage 或击败，不能靠士气伤害刷。
...
| Rank3 | MaxHp +2，Attack +1，HP 全回复。解锁 `starfall`：2 AP，选择敌方 1 体，造成 2 点魔法伤害并赋予 1 层燃烧；若 Mage 当前有咏唱，消耗 1 层，改为造成 4 点魔法伤害。Cooldown 1。 |

平衡边界：

- `starfall` 是普通魔法伤害，先扣士气；不是绝对伤害。
...
| Rank3 | MaxHp +2，Attack +1，HP 全回复。解锁 `archive-formula`：1 AP，选择敌方 1 体，目标获得 `archived`。本 turn 下一次我方 Role Action 对其造成伤害或施加 Debuff 后，额外造成 1 点魔法伤害并赋予 1 层燃烧。Cooldown 1。 |

平衡边界：

- `archived` 是特殊印记，不进入通用 Debuff 命名池；可以被净化。
- 额外 1 点魔法伤害是普通伤害，先扣士气。
...
| Rank3 | MaxHp +4，PhysicalDefense +1，HP 全回复。解锁 `grove-sanctuary`：2 AP，选择我方 1 体及相邻我方，各净化 1 个 Debuff；每个成功净化的目标治疗 2 HP，未净化目标治疗 1 HP。Cooldown 1。 |

平衡边界：

- 净化成功可以触发 Cleric 体系；未净化的治疗不能伪装成净化触发。
- 治疗不回复士气，不改变部署中限制。
- 该路线克制状态，但主动伤害弱。
...
| Rank3 | MaxHp +3，Attack +1，HP 全回复。解锁 `call-the-hunt`：1 AP，选择敌方 1 体，目标获得 `hunted`。本 turn 前两次我方 Soldier 对该目标造成普通伤害时，最终伤害 +1；若该目标本 turn 被击败，我方获得 1 BP。Cooldown 1。 |

平衡边界：

- `call-the-hunt` 加的是普通伤害，仍先扣士气。
...
| Rank2 | MaxHp +2，Attack +1。Trait 强化：Barbarian 处于狂怒时触发 `aftershock-axe`，余波伤害最低为 2；仍必须先造成至少 3 点真实 HP 伤害。 |
| Rank3 | MaxHp +3，Attack +1，HP 全回复。解锁 `glory-roar`：1 AP，自身获得强攻 1 turn 和 1 次额外主动攻击；本 turn 第一次主动攻击后，自身失去 2 HP。Cooldown 1。 |

平衡边界：

- 失去 HP 直扣 HP，不经过士气。
- `war-cry` 破盾追加是绝对伤害，直扣 HP；共享盾本身仍不穿透。
...
| Rank3 | MaxHp +3，Attack +1，HP 全回复。解锁 `dragon-breaker`：1 AP，选择敌方 1 体。若敌方有共享盾，对其共享盾造成 3 点伤害但不穿透 HP；目标获得 1 turn 战栗和 1 turn 脆弱。Cooldown 1。 |

平衡边界：

- `dragon-breaker` 的盾伤害不造成 HP 伤害，不能触发 HPDamage 奖励。
...
| Rank3 | MaxHp +3，Attack +1，HP 全回复。解锁 `nightmare-stare`：1 AP，选择敌方 1 体，目标和相邻敌人获得猎物；主目标额外获得 1 turn 磨损。Cooldown 1。 |
...
| Rank3 | MaxHp +4，PhysicalDefense +1，HP 全回复。解锁 `abyssal-bargain`：1 AP，选择我方 1 体，目标失去最多 3 HP，获得契约并可再次主动攻击；该目标本 turn 第一次造成敌方 HP 伤害时，我方获得 1 BP。Cooldown 1。 |
...
| Rank3 | MaxHp +4，PhysicalDefense +1，HP 全回复。解锁 `holy-bastion`：2 AP，选择我方 1 体，目标获得 2 层守护誓约和 1 turn 护咒；Knight 获得 1 turn 坚守。Cooldown 1。 |

平衡边界：

- 守护誓约仍只对主动物理攻击生效。
- 该路线强防守但不直接处理士气；需要队友输出。
- 共享盾不穿透的新规则会提高防线稳定性，Rank3 不再额外增盾。
...
| Rank3 | MaxHp +3，Attack +1，HP 全回复。解锁 `iron-charge`：1 AP，消耗我方最多 3 点共享盾，对敌方 1 体造成等量物理伤害并施加 1 turn 战栗；若没有共享盾则不能发动。Cooldown 1。 |

平衡边界：

- `iron-charge` 是普通物理伤害，先扣士气。
