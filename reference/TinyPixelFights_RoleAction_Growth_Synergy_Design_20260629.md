...
| Rank3 | MaxHp +3，MagicalDefense +1，HP 全回复。解锁 `thread-cut`：1 AP，选择敌方 1 体。统计目标身上的特殊印记与通用 Debuff 数量，每个可计数状态造成 2 点士气伤害，不设数量上限。士气伤害溢出不穿透 HP；若目标士气因此归 0，再造成 Oracle 当前 Attack 点魔法伤害并施加 1 turn 战栗。若目标没有任何可计数状态，改为只施加 `marked`。Cooldown 1。 |
