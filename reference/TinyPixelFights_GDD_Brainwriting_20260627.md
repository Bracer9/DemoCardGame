...
| 局内成长层 | BP、固定奖励窗口、英雄升级、士兵军衔 | 如何让一局内形成 JRPG 成长曲线 |
...
曾经考虑过让每个玩家在一场对战内拥有独立“指挥等级”：BP 累计到阈值后升级，升级后解锁三选一奖励。

### 6.2 当前结论：不进入当前对战 prototype

这个方向暂时否决。原因不是“不酷”，而是它把太多系统耦合在一起：

- BP 既是经验又是货币。
- 奖励触发依赖等级阈值。
- 奖励强度又反过来影响 BP 获取。
- UI 需要解释等级、累计 BP、可用 BP、奖励可领取状态。
- 平衡表需要同时模拟等级曲线、奖励价格、奖励概率、领先者滚雪球和落后方补偿。

对当前项目来说，这会让数值计划的复杂度指数型增长，远超第一阶段 prototype 能安全承受的范围。
...
```text
局内成长需要节奏器，但节奏器不一定是额外进度系统。
```

当前替代方案：

```text
战斗行动 → 获得 BP → 固定 round 奖励窗口 → 花 BP 购买三选一奖励
```

奖励时机由 round 控制，奖励购买由 BP 控制。这样成长节奏可预测，BP 仍有取舍，但不需要额外等级系统。
...
BP 不再推进等级。它是局内奖励消费货币，用于固定 round 奖励窗口中的三选一奖励购买。
...
为了对战公平和可读，第一版不建议使用击杀奖励，也不建议把治疗、承伤、专属技能都塞进 BP 来源。
...
- 低保保证双方都能参与奖励系统。
- 破盾奖励让共有盾不只是拖延，也成为可争夺资源。
- 主动伤害奖励鼓励推进，但上限防止一回合爆炸。
- 不给击杀额外 BP，避免领先者击杀后继续滚雪球。
...
## 8. 机制 5：固定 Round 奖励三选一

### 8.1 原始想法

奖励选择借鉴肉鸽式三选一，但不再由额外进度系统触发，而是由固定 round 节奏触发。奖励需要消耗 BP。
...
双方开局各自获得 5 BP。
Round 1 触发第一次奖励选择。
之后每隔 2 个 round 触发一次奖励选择。
每次奖励窗口显示三选一。
每次奖励窗口有 1 次 reset。
玩家可以跳过奖励以保存 BP。
选择奖励不消耗 AP，支付 BP 后立即生效。
```
...
- 三选一中至少一项永远是稳定通用奖励。
- 玩家可选择跳过，不花 BP、不拿奖励，保留资源给下次奖励窗口。
...
固定奖励窗口中可能出现已经持有的普通兵。选择并支付 BP 后提升该普通兵军衔。
...
固定奖励窗口中可能出现类似 STS relic 的遗物或装备。它们提供持续效果，例如物理单位全员攻击 +1。
...
| 机制 | 原因 |
---|---|
| BP 奖励窗口 + 三选一 | 需要新 UI、奖励池、价格与节奏控制 |
...
### Prototype 3：固定奖励窗口与 BP 消费

目标：

- 验证固定 round 奖励窗口与 BP 消费是否好玩。

范围：

- 初始 5 BP，上限 10 BP。
- Round 1 触发第一次奖励，之后每隔 2 round 触发。
- 每回合低保 +1 BP，破盾 +1 BP，主动攻击造成 HP 伤害 +1 BP。
- 每回合最多获得 3 BP。
- 奖励只有小属性、普通兵成长和一次英雄技能解锁。
- 每次奖励窗口三选一、可 reset 一次、可跳过攒 BP。
...
不要直接把转职、副官写成正式规则；当前只把 BP 奖励窗口写成近期 prototype，其余写成候选系统：
...
- 重型成长进度方案已被检讨并暂时放弃；BP、英雄升级、普通兵、副官、遗物、天气仍是候选或分阶段 prototype 系统。
...
4. 英雄升级应定价为多少 BP，才会让玩家在“立刻买”和“跳过攒资源”之间纠结？
5. 普通兵是否会死亡，还是只在本场退场？
6. 副官附着后是否可被攻击/移除？
...
> Tiny Pixel Fights 的下一阶段不应该只是“给现有 8 张英雄加更多技能”，而应该让对战模式逐步验证一个更有生命力的局内成长结构：普通兵承载低复杂度战斗，英雄承载身份与转职，固定奖励窗口与 BP 消费承载局内成长节奏，遗物与天气承载变化战场。真正的挑战不是想法不够，而是每次只让一个系统进入 prototype，并确保玩家始终看得懂自己为什么赢、为什么输、为什么变强。
