...
- **指挥等级**：暂缓作为第一阶段系统。先不引入重型等级条，避免把成长乐趣和复杂经济系统绑死。
- **BP**：可以保留为轻量成长货币。第一版倾向于低保式获得，并少量奖励主动推进战斗，例如回合补给 + 首次有效进攻奖励。BP 不应成为刷分游戏，也不应让领先者滚雪球。
- **奖励选择**：倾向于固定 round 节奏触发三选一奖励，每次可获得一次刷新机会。关键奖励如英雄升级可以有隐藏保底或权重修正，但不要求玩家阅读概率规则。
...
- 每隔固定 round 或关键节奏点，双方进入奖励选择。
- 每名玩家看到 3 个奖励选项，并拥有一次刷新机会。
- 奖励按稀有度和条件随机出现，但关键成长奖励需要隐藏保底或权重保护，避免玩家完全抽不到核心成长。
- BP 若加入，应是轻量成长货币：主要通过稳定补给获得，少量奖励主动推进战斗。
- BP 不应作为复杂表现评分，也不应鼓励玩家为了刷 BP 放弃合理战术。

可测试的 BP 方向：

```text
+1 BP：回合或 round 补给。
+1 BP：本回合首次造成生命伤害或击破敌方共有盾。
主动奖励每回合最多一次。
```
