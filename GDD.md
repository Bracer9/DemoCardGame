...
- 当前 AI 对战定位仍是陪练与系统验证，不追求最优解算器；开局可选择 **Easy / Normal** 两档难度。
- **Easy** 保留轻量构筑倾向和加权随机，只使用攻击、共有盾、副官及成长选择，不主动使用 Role Action。
- **Normal** 沿用 Easy 的构筑随机性，但会根据现有 Role Action 预测评估伤害、治疗、防御、额外攻击、AP 代价和目标状态；每回合最多主动使用一次，并把首次 Role Action 可得 BP 纳入判断。
- Normal 在奖励窗口会识别免费升级、缺员招募、士兵成长和契合遗物；当没有关键购买且 BP 偏低时，可以跳过窗口积攒 BP。该行为仍带随机性，不保证全局最优。
- AI 使用英雄路线、遗物和副官共用的 Build Tags 形成轻量构筑倾向，影响英雄 / 士兵招募、英雄路线、遗物和副官宿主选择。
- Build Tags 只作为加权随机因素；协同选项出现概率更高，但其他合理选项仍可能被选择，让 AI 保留失误、变化和不同局势。
- 副官搭配只排除物攻副官交给魔法英雄、魔攻副官交给物理英雄等明确无效组合，不建立逐英雄 × 逐副官的专属相性表。
- 新增英雄路线、士兵副官或遗物时，应维护通用 Build Tags，而不是继续向 AI 添加角色 ID 特判。

### 当前暂定机制口径

- **等级条成长方案**：不进入当前对战 prototype。这个方向曾被检讨过，但会显著放大奖励、经济、UI 和数值计划复杂度，现阶段只保留为被放弃的设计备忘。
- **BP**：保留为轻量成长货币和奖励消费资源。当前对战测试口径是开局 5 BP、上限 20 BP；BP 不再推进等级，而是直接用于固定奖励窗口中的三选一奖励购买。
- **奖励选择**：固定 round 节奏触发三选一奖励。Round 4 触发第一次奖励选择，之后每隔 4 个 round 再触发一次。每次奖励选择可跳过以保存 BP，并拥有 1 次刷新机会。关键奖励如英雄升级可以有隐藏保底或权重修正，但不要求玩家阅读概率规则。
- **英雄成长**：优先验证第一次升级二选一，暂缓完整三段升级、转职和新立绘。
- **普通兵成长**：先验证普通兵是否能降低认知负担并支撑小队结构，再加入军衔、副官和兵种技能。
- **遗物**：当前已接入 26 件遗物，由 16 件跨体系 Common / Rare 组件与 10 件 Epic 构筑终点组成；完整随机池仅按英雄路线 tag 做宽松加权，不设置路线硬门槛。详细口径见 `reference/TinyPixelFights_Relic_Design_20260708.md`。天气仍属于后续全局 modifier 系统。
...
角色攻击时，攻击方和防守方会根据当前规则结算伤害与反击。确认攻击前会显示预测；具有立即数值效果的 Role Action 也必须在确认前显示当前目标下的数值预测。攻击、反击和 Role Action 伤害会受到以下因素影响：
...
- 如果盾仍存在，再次使用会强化盾值。
- 盾会被伤害击破。
- 共有盾只能吸收等同当前盾值的伤害。超过剩余盾值的部分会穿透，并继续经过目标自身的物理/魔法防御、士气与 HP 结算。
- 绝对伤害默认无视共有盾；明确写明“只削减共有盾、不造成 HP 伤害”的技能属于独立例外，不使用普通伤害穿透流程。
- 怪物与猎物系绝对追击只在本次伤害既未削减士气、也未削减 HP 时触发；只造成士气伤害不再触发。
...
- 双方开局各自获得 **5 BP**，BP 上限为 **20**。
- **Round 4** 触发第一次奖励选择，之后每隔 **4 个 round** 触发一次奖励选择。
...
| 日期 | 修改 |
| --- | --- |
| 2026年7月13日 | 共有盾改为只吸收当前盾值、溢出伤害继续结算；攻击与立即数值型 Role Action 均须提供结算前预测；同步校正当前 BP、奖励节奏与遗物数量。 |
